...
import TransparentHeader from '../home/TransparentHeader';
...
function Showcases({ showcase, isDark, isAdmin }) {
  return (
    <Carousel animation="slide" navButtonsAlwaysVisible={false} fullHeightHover="true" swipe={false}>
      {showcase.map((item, i) => (
        <ShowcasesItem isDark={isDark} key={i} item={item} isAdmin={isAdmin} />
      ))}
    </Carousel>
  );
}
      ...
      <Stack direction="row" spacing={2} style={{ height: '100%', overflow: 'hidden' }} justifyContent="flex-end">
        <Stack
          ...
          <Stack direction="row" spacing={2} justifyContent="flex-start">
            ...
            <Link
              to={'/cosmos-ui/market-listing/cosmos-cloud/' + item.name}
              style={{
                textDecoration: 'none'
              }}>
...
const MarketPage = () => {
  ...
  let appList =
    apps &&
    Object.keys(apps).reduce((acc, appstore) => {
    ...
    <>
        ...
        <Box
          ...
          <Stack direction="row" spacing={2} style={{ height: '100%' }} justifyContent="flex-end">
            <Stack
              ...
              <Stack direction="row" spacing={2}>
                ...
                <h2>
                  ...
                  <span style={{ color: 'grey' }}>
                    {openedApp.appstore != 'cosmos-cloud' ? ' @ ' + openedApp.appstore : ''}
                  </span>
                </h2>
              </Stack>

              <div>{openedApp.tags && openedApp.tags.slice(0, 8).map((tag) => <Chip label={tag} />)}</div>

              <div>
                {openedApp.supported_architectures &&
                  openedApp.supported_architectures.slice(0, 8).map((tag) => <Chip label={tag} />)}
              </div>
              ...
              {openedApp.appstore != 'cosmos-cloud' && (
                <div>
                  <div>
                    ...
                    <strong>source:</strong> {openedApp.appstore}
                  </div>
                </div>
              )}
              ...
              <div>
                <div>
                  <strong>repository:</strong> <LinkMUI href={openedApp.repository}>{openedApp.repository}</LinkMUI>
                </div>
                <div>
                  <strong>image:</strong> <LinkMUI href={openedApp.image}>{openedApp.image}</LinkMUI>
                </div>
                <div>
                  <strong>compose:</strong> <LinkMUI href={openedApp.compose}>{openedApp.compose}</LinkMUI>
                </div>
              </div>
              ...
              {isAdmin && (
                <div>
                  <DockerComposeImport
                    installerInit
                    defaultName={openedApp.name}
                    dockerComposeInit={openedApp.compose}
                  />
                </div>
              )}
            </Stack>
      ...
      <Stack style={{ position: 'relative' }} spacing={1}>
        <Stack style={{ height: '35vh' }} spacing={1}>
          {(!showcase || !Object.keys(showcase).length) && (
            <Box
              style={{
                width: '100%',
                height: '100%',
        ...
        <Stack
          ...
          <Stack direction="row" spacing={2}>
            <Input
              placeholder={'Search ' + filteredAppList.length + ' applications'}
              value={search}
            ...
            <Link to="/cosmos-ui/servapps/new-service">
              <ResponsiveButton variant="contained" startIcon={<AppstoreAddOutlined />}>
                Start ServApp
              </ResponsiveButton>
            </Link>
              ...
              {filteredAppList.map((app) => {
                return (
                  <Grid2
                    ...
                    lg={4}
                    xl={3}
                    key={app.name + app.appstore}
                    item>
                    <Link
                      to={'/cosmos-ui/market-listing/' + app.appstore + '/' + app.name}
                      style={{
                        textDecoration: 'none'
                      }}>
                        ...
                        <Stack spacing={3} direction={'row'} alignItems={'center'} style={{ padding: '0px 15px' }}>
                          ...
                          <Stack spacing={1}>
                            <div style={{ fontWeight: 'bold' }}>
                              {app.name}
                              <span style={{ color: 'grey' }}>
                                {app.appstore != 'cosmos-cloud' ? ' @ ' + app.appstore : ''}
                              </span>
                            </div>
                            <div
                              style={{
